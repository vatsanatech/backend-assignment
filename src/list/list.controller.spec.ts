...
describe('UserController', () => {
    ...
    describe('create', () => {
        ...
        it('should throw a ConflictException if the user already exists', async () => {
            jest.spyOn(service, 'addToList').mockRejectedValue(new ConflictException());

            await expect(controller.create(mockUserDto)).rejects.toThrow(ConflictException);
        });
    });
    ...
    describe('removeUser', () => {
        ...
        it('should throw a NotFoundException if the user does not exist', async () => {
            jest.spyOn(service, 'removeFromList').mockRejectedValue(new NotFoundException());

            await expect(controller.removeUser('someUserId')).rejects.toThrow(NotFoundException);
        });
    });
    ...
    describe('findAll', () => {
        it('should return a list of users', async () => {
            ...
            jest.spyOn(service, 'listUser').mockResolvedValue({ users: mockUsers, next: 1 });
